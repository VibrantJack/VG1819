...
void main()
{
	...
	if( distance1 < Light1Range )
	{
		...
		// Specular
		vec3 R = reflect(-Light1Dir,n);
		float specFactor = pow(max(dot(R, ViewDir), 0.0), MatSpecular.w);
		vec4 specular = specFactor * MatSpecular * Light1Specular;

		// Combine the components
		//vec4 Light1 = clamp(ambient + diffuse + specular, 0, 1);
	...
	if( distance2 < Light2Range )
	{
		...
		float specFactor = pow(max(dot(R, ViewDir), 0.0), MatSpecular.w);
		vec4 specular = specFactor * MatSpecular * Light2Specular;

		// Combine the components
		//vec4 Light2 = clamp(ambient + diffuse + specular, 0, 1);
