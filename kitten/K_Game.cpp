	...
	bool initGame()
	{
		...
		*/
		...
		// Networking
		{
			...
			// Create GO, resize and place
			K_GameObject* networkMenuGO = K_GameObjectManager::getInstance()->createNewGameObject();
			kitten::Transform* parent = &networkMenuGO->getTransform();
			networkMenuGO->getTransform().scale2D(128.0, 72.0);
			networkMenuGO->getTransform().place2D(0.0, 0.0);

			// Create UIFrame, set texture to network menu background
			userinterface::UIFrame* menuFrame = static_cast<userinterface::UIFrame*>(compMan->createComponent("Frame"));
			menuFrame->setTexture("textures/ui/network_menu_bg.tga");
			networkMenuGO->addComponent(menuFrame);

			// Add console menu component for use with the buttons
			networkMenuGO->addComponent(compMan->createComponent("NetworkingConsoleMenu"));			

			// Creating Join Button GO, has: UIFrame for texture, ClickableFrame and NetworkJoinButton(ClickableUI) for clicks
			kitten::K_GameObject* joinButtonGO = kitten::K_GameObjectManager::getInstance()->createNewGameObject();
			joinButtonGO->getTransform().setParent(parent);
			joinButtonGO->getTransform().scale2D(170, 100);
			joinButtonGO->getTransform().place2D(690, 310);

			// Creating Frame component for texture
			userinterface::UIFrame* joinButtonFrame = static_cast<userinterface::UIFrame*>(compMan->createComponent("Frame"));
			joinButtonFrame->setTexture("textures/ui/join_button.tga");
			joinButtonGO->addComponent(joinButtonFrame);

			// Creating ClickableFrame and NetworkJoinButton(ClickableUI)
			kitten::ClickableFrame* jcfCasted = static_cast<kitten::ClickableFrame*>(compMan->createComponent("ClickableFrame"));
			userinterface::NetworkJoinButton* joinButtonComponent = static_cast<userinterface::NetworkJoinButton*>(compMan->createComponent("NetworkJoinButton"));
			jcfCasted->addCLickable(joinButtonComponent);
			joinButtonGO->addComponent(jcfCasted);
			joinButtonGO->addComponent(joinButtonComponent);

			// Creating Host Button GO, same as Join Button GO, but with NetworkHostButton
			kitten::K_GameObject* hostButtonGO = kitten::K_GameObjectManager::getInstance()->createNewGameObject();
			hostButtonGO->getTransform().setParent(parent);
			hostButtonGO->getTransform().scale2D(170, 100);
			hostButtonGO->getTransform().place2D(420, 310);

			// Creating Frame component for texture
			userinterface::UIFrame* hostButtonFrame = static_cast<userinterface::UIFrame*>(compMan->createComponent("Frame"));
			hostButtonFrame->setTexture("textures/ui/host_button.tga");
			hostButtonGO->addComponent(hostButtonFrame);

			// Creating ClickableFrame and NetworkJoinButton(ClickableUI)
			kitten::ClickableFrame* hcfCasted = static_cast<kitten::ClickableFrame*>(compMan->createComponent("ClickableFrame"));
			userinterface::NetworkJoinButton* hostButtonComponent = static_cast<userinterface::NetworkJoinButton*>(compMan->createComponent("NetworkHostButton"));
			hcfCasted->addCLickable(hostButtonComponent);
			hostButtonGO->addComponent(hcfCasted);
			hostButtonGO->addComponent(hostButtonComponent);

			// Visual box for entering IP address
			kitten::K_GameObject* ipBoxOutline = kitten::K_GameObjectManager::getInstance()->createNewGameObject();
			ipBoxOutline->getTransform().scale2D(250, 36);
			ipBoxOutline->getTransform().place2D(515, 470); //-10, -36
			userinterface::UIFrame* ipBoxFrame = static_cast<userinterface::UIFrame*>(compMan->createComponent("Frame"));
			ipBoxFrame->setTexture("textures/ui/ip_box.tga");
			ipBoxOutline->addComponent(ipBoxFrame);

			// Getting string input
			kitten::K_GameObject* textInput = kitten::K_GameObjectManager::getInstance()->createNewGameObject("StringInputDisplay.txt");
